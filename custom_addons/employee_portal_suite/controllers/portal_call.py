...
def _binary_image_data_uri(value):
    """Return an Odoo Binary image value as a browser-safe data URI.

    Binary image fields are already base64-encoded by the ORM.  Keep this
    helper local so the call feature does not depend on an Odoo tools export
    that differs between server builds.
    """
    if not value:
        return ''
...
class PortalCallController(http.Controller):
    ...
    def _employee_avatar(self, user):
        ...
        if employee and employee.image_128:
            return _binary_image_data_uri(employee.image_128)
        if getattr(user, 'avatar_128', False):
            return _binary_image_data_uri(user.avatar_128)
        return ''
    ...
    def call_contacts(self):
        ...
        for employee in employees:
            ...
            result.append({
                ...
                'avatar': _binary_image_data_uri(employee.image_128) if employee.image_128 else (_binary_image_data_uri(contact.avatar_128) if contact.avatar_128 else ''),
            })
        return result
