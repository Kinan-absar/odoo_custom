...
class EmployeePortalAnnouncements(http.Controller):
    ...
    def _get_accessible_announcement_attachment(self, announcement_id, attachment_id):
        announcement = request.env['portal.announcement'].sudo().browse(announcement_id)
        if not announcement.exists() or not announcement._user_can_access(request.env.user, target='portal'):
    ...
    def portal_announcement_attachment_view(self, announcement_id, attachment_id, **kw):
        announcement, attachment = self._get_accessible_announcement_attachment(announcement_id, attachment_id)
