...
class EmployeePortalNativeDiscussController(http.Controller):
    ...
    def _mark_channel_seen(self, channel, user):
        """Synchronise portal read state with native Discuss membership.

        Native Discuss unread counters are driven by the member's seen message, not
        by our portal list.  Mark against the latest channel message of any type so
        system/notification messages cannot keep the member artificially unread.
        """
        ...
        # Keep fetched/seen pointers aligned when those native fields are present.
        # This is especially important for portal-opened standard channels where
        # the public Discuss store does not always send the same read receipt as
        # the backend web client.
        vals = {}
        if 'seen_message_id' in member._fields and (not member.seen_message_id or member.seen_message_id.id < latest.id):
            vals['seen_message_id'] = latest.id
        if 'fetched_message_id' in member._fields and (not member.fetched_message_id or member.fetched_message_id.id < latest.id):
            vals['fetched_message_id'] = latest.id
        if vals:
    ...
    def _discuss_home_values(self, user):
        ...
        for channel in channels:
            ...
            is_group = (not is_channel) and (channel.channel_type == 'group' or len(self._channel_users(channel)) > 2)
