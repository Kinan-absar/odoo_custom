...
class DiscussChannel(models.Model):
    ...
    def message_post(self, **kwargs):
        ...
        # Native Discuss remains the source of truth. Web Push/Telegram are only
        # external alerts. Employee DMs/groups are explicitly marked; regular
        # Odoo channels are also eligible when they contain portal employee members.
        Employee = self.env['hr.employee'].sudo()
        def _portal_visible_channel(channel):
            if channel.is_employee_portal_channel:
                return True
            if channel.channel_type != 'channel':
                return False
            member_users = self.env['res.users'].sudo().search([
                ('active', '=', True),
                ('share', '=', True),
                ('partner_id', 'in', channel.channel_member_ids.partner_id.ids),
            ])
            return bool(member_users and Employee.search_count([
                ('active', '=', True), ('user_id', 'in', member_users.ids),
            ]))

        for channel in self.sudo().filtered(_portal_visible_channel):
            ...
            if channel.channel_type == 'channel':
                users = users.filtered(lambda u: u.share)
