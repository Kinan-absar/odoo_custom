...
- Approver users/groups/record field are selectable directly in the inline stage grid.
- Current approvers are stored in a dedicated relation table, eliminating custom-search domain rewriting errors.
- Approver field filtering uses a related target model field and only supports user Many2one/Many2many fields.
- User selectors show active internal Odoo users only. Employees without an Odoo user cannot approve until a user account is linked.
- Workflow chatter and safer overdue searching added.
