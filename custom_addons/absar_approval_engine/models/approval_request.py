...
class AbsarApprovalRequest(models.Model):
    ...
    current_approver_ids = fields.Many2many(
        'res.users', 'absar_request_current_user_rel', 'request_id', 'user_id',
        compute='_compute_current_approvers', store=True, readonly=True,
    ...
    def _search_is_overdue(self, operator, value):
        positive = (operator in ('=', '==') and bool(value)) or (operator in ('!=', '<>') and not value)
        overdue_domain = [
            ('state', '=', 'in_progress'),
            ('deadline', '!=', False),
