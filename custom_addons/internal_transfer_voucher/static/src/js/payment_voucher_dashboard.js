...
class PaymentVoucherDashboard extends Component {
    ...
    setup() {
        ...
        this.data = useState({
            draft_count: 0, posted_count: 0, cancel_count: 0,
            cash_count: 0, cheque_count: 0, bank_count: 0, transfer_count: 0,
            total_posted_amount: 0,
            my_draft_count: 0, my_posted_count: 0, my_cancel_count: 0,
            my_cash_count: 0, my_cheque_count: 0, my_bank_count: 0, my_transfer_count: 0,
            my_total_posted_amount: 0,
            currency_symbol: "",
        });
        onWillStart(async () => {
            const result = await this.orm.call("account.payment.voucher", "retrieve_dashboard", []);
    ...
    get myDomain() { return [["create_uid", "=", user.userId]]; }
...
class PaymentVoucherListController extends ListController {
    ...
    static components = { ...ListController.components, PaymentVoucherDashboard };
