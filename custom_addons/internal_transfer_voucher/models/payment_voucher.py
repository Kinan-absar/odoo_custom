        ...
        for rec in self:
            ...
            if rec.move_id and rec.move_id.state == 'posted':
                try:
                    ...
                    # Odoo can block button_draft() on posted moves that contain
                    # dynamic tax lines because resetting them may delete/rebuild
                    # tax lines already considered by the tax report.
                    # These moves are generated by this voucher module, so for this
                    # specific tax-line validation we safely fall back to the old
                    # direct state reset while keeping the existing move_id linked.
                    error_message = str(error)
                    if (
                        'tax line' in error_message.lower()
                        and 'tax report' in error_message.lower()
                    ):
